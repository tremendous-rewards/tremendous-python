...
class CreateOrder200ResponseOrder(BaseModel):
    ...
    rewards: Optional[List[CreateOrder200ResponseOrderRewardsInner]] = None
    ...
    def status_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['CANCELED', 'OPEN', 'EXECUTED', 'FAILED', 'PENDING APPROVAL', 'PENDING INTERNAL PAYMENT APPROVAL']):
            raise ValueError("must be one of enum values ('CANCELED', 'OPEN', 'EXECUTED', 'FAILED', 'PENDING APPROVAL', 'PENDING INTERNAL PAYMENT APPROVAL')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "payment": ListOrders200ResponseOrdersInnerPayment.from_dict(obj["payment"]) if obj.get("payment") is not None else None,
            "invoice_id": obj.get("invoice_id"),
            "rewards": [CreateOrder200ResponseOrderRewardsInner.from_dict(_item) for _item in obj["rewards"]] if obj.get("rewards") is not None else None
        })
        return _obj
